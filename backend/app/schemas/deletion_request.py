...
class DeletionRequestBase(BaseModel):
    user_id: uuid.UUID = Field(
        ..., description="The ID of the user whose data is requested for deletion."
    )
...
class DeletionRequestUpdate(BaseModel):
    status: DeletionRequestStatus = Field(
        ..., description="The new status of the deletion request."
    )
    purged_at: datetime | None = Field(
        None, description="Timestamp when the data purge was completed."
    )
    failure_reason: str | None = Field(
        None, description="Reason for deletion failure, if applicable."
    )
