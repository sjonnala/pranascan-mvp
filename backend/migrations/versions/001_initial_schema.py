...
def upgrade() -> None:
    ...
    if not _has_index("consent_records", "ix_consent_records_user_id"):
        op.create_index("ix_consent_records_user_id", "consent_records", ["user_id"])

    if not _has_table("scan_sessions"):
        op.create_table(
            "scan_sessions",
            sa.Column("id", sa.String(36), primary_key=True),
            sa.Column("user_id", sa.String(36), nullable=False, index=True),
    ...
    if not _has_index("scan_sessions", "ix_scan_sessions_user_id"):
    ...
    if not _has_index("scan_results", "ix_scan_results_user_id"):
