...
# ---------------------------------------------------------------------------
# Integration: calibration wired into scan pipeline (smoke test via API)
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_scan_result_includes_skin_tone_flags(client, auth_headers):
    """Completed scan with frame_data should include skin_tone flags in result."""
    # Grant consent first
    await client.post(
        "/api/v1/consent",
        json={"user_id": "00000000-0000-0000-0000-000000000001",
              "consent_version": "1.0", "purpose": "wellness_screening"},
    ...
    # Build 30 synthetic frames (medium skin tone RGB)
    frame_data = [
        {"t_ms": i * 333, "r_mean": 175.0, "g_mean": 130.0, "b_mean": 95.0}
        for i in range(30)
    ]
    ...
    complete_resp = await client.put(
        ...
        json={
            "hr_bpm": 72.0, "hrv_ms": 45.0, "respiratory_rate": 16.0,
            "voice_jitter_pct": 0.5, "voice_shimmer_pct": 2.0,
            "quality_score": 0.92, "lighting_score": 0.85,
            "motion_score": 0.98, "face_confidence": 0.95,
            "audio_snr_db": 25.0, "flags": [],
            "frame_data": frame_data,
        },
        headers=auth_headers,
    )
    assert complete_resp.status_code == 200, complete_resp.text
    result_flags = complete_resp.json().get("flags", [])
    assert any("skin_tone_fitzpatrick_type_" in f for f in result_flags), (
        f"Expected skin_tone flag in result flags. Got: {result_flags}"
    )
    assert "skin_tone_calibration_applied" in result_flags
